...
public class MixChannelSSDTest extends MixChannelTest {
    @BeforeAll
    public static void setup() throws IOException {
        parent = tempDirectory();
        firstLayerPath = parent.toString() + "/ssd";
        secondLayerPath = parent.toString() + "/hdd";
        File topicDir = new File(secondLayerPath + "/" + TOPIC);
        assertTrue(topicDir.mkdirs());

        firstLayerMode = Mode.SSD;
        PMemChannel.init(firstLayerPath, Integer.toString(SIZE), BLOCK_SIZE, 0.01);
        MixChannel.init(firstLayerPath, firstLayerMode, secondLayerPath, secondLayerMode, Integer.toString(SIZE), 0.9, 1);
    }
}
